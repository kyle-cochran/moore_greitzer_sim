...
def build_linear_system(u, dt, dx, D = 3, P = 3,time_diff = 'poly',space_diff = 'poly',lam_t = None,lam_x = None, width_x = None,width_t = None, deg_x = 5,deg_t = None,sigma = 2, time_diff_in=None):
    """
...
def TrainSTRidge(R, Ut, lam, d_tol, maxit = 25, STR_iters = 10, l0_penalty = None, normalize = 2, split = 0.8, print_best_tol = False):
    ...
    if np.isinf(TrainR).any() or np.isnan(TrainR).any():print("trainR bad")
    if np.isinf(TrainY).any() or np.isnan(TrainY).any():print("trainY bad")
    if np.isinf(TestR).any() or np.isnan(TestR).any():print("TestR bad")
    if np.isinf(TestY).any() or np.isnan(TestY).any():print("TestY bad")
    print(err_best)
    print(w_best)
    print(np.count_nonzero(w_best))
    print(TestY)
    print(TestR)
    tol_best = 0
...
def STRidge(X0, y, lam, maxit, tol, normalize = 2, print_results = False):
    ...
    bada = X0
    badb = y
    if np.isnan(bada).any() or np.isinf(bada).any(): print("X0")
    if np.isnan(badb).any() or np.isinf(badb).any(): print("y")
    # First normalize data
    if normalize != 0:
        Mreg = np.zeros((d,1))
        for i in range(0,d):
    ...
    if lam != 0:
        try:
            ...
            print("bad val: ")
            bada = X.T.dot(X) + lam*np.eye(d)
            badb = X.T.dot(y)

            if np.isnan(bada).any() or np.isinf(bada).any(): print("a")
            if np.isnan(badb).any() or np.isinf(badb).any(): print("b")
